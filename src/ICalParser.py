...
class ICalParser:
    ...
    def __init__(self):
        ...
        self.OccurencesWindowStartDate = ""
        # Date until which occurences of iCalendar events should be returned by the enumerator
        self.OccurencesWindowEndDate = ""
        #the VCALENDAR as a string
        self.sVCALENDAR = []
        #the VCALENDAR as a typed object
        self.dVCALENDAR = {}
        #the current VEVENT being loaded from iCalendar file, array of strings each string is an unfolded
        self.lVEVENT = []
...
#        self.local_path = path
        ...
        string = open(sLocalFilePath,'r',encoding='utf-8', errors='ignore').readlines()
        #FIXME: add here the CRLF check and remove the \n from strings_load
        #RFC5545_SCM["3.1_1"]
        self.strings_load(string,conformance,append)

    def from_string(self, ics_str, conformance=False, append=False):
        lines_array = ics_str.split("\r\n")
        self.strings_load(lines_array, conformance, append)
        ...
        if "DTSTART" not in dVevent:
            if "RRULE" in dVevent:
                ...
                if "UID" in dVevent:
                    ...
                    uid = "not available"
                self.Validator("3.6.1_0", "event UID is: .. rd removed due to error",level=1)
                pass
...
#        self.event = []
    ...
    def _flatten(self):
        ...
        for event in self.events:
            tmp_event = event
            if "DTSTART" in tmp_event:
                tmp_event["DTSTART"]=tmp_event["DTSTART"]["val"]
            elif "Time" in tmp_event and len(tmp_event["Time"]) > 0:
                timeStr = tmp_event["Time"][0]
                if type(timeStr) is not types.DictType:
                    timeStr = timeStr[0]
                timeStr = timeStr['val']
                if type(timeStr) is types.StringType:
                    retdate = datetime.datetime.strptime(timeStr[:15],"%Y%m%dT%H%M%S")
                    tmp_event["DTSTART"]=retdate
                else:
                    tmp_event["DTSTART"]=datetime.datetime.now()
            ...
            if "LOCATION" in tmp_event:
                tmp_event["LOCATION"]=tmp_event["LOCATION"]["val"]
            else:
                tmp_event["LOCATION"] = ""
            ...
            # Rob calc duration
            try:
                duration = tmp_event["DTEND"] - tmp_event["DTSTART"]
            except:
...
#            t_res = self._flatten_rrule(event)
            ...
            for t_date in t_res:
                if len(self.events_instances)==0:
                    ...
                    self.events_instances=[[t_date,duration,summary,location,tmp_event["UID"]["val"]]]
                else:
                    #if already instances in the list
                    self.events_instances.append([t_date,duration,summary,location,tmp_event["UID"]["val"]])
...
#                            print "dw, dwi, min, max", dw, dwi, min, max
        ...
        for t_date in list_dates:
            if t_date>=event_start and  t_date>=self.OccurencesWindowStartDate and t_date<=self.OccurencesWindowEndDate and t_date<=event_end:
                ...
                t_date = t_date.replace(hour=dtstart.hour, minute=dtstart.minute, second=dtstart.second)
                if MaxCount >0:
