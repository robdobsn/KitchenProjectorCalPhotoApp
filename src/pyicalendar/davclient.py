...
class DAVClient:
    """
    Basic client for webdav, heavily based on http.client
    """
    proxy = None
    url = None
    ...
    def __init__(self, url, proxy=None):
        ...
        if self.proxy is not None:
            self.handle = http.client.HTTPConnection(*self.proxy)
        # direct, https
        elif self.url.port == 443 or self.url.scheme == 'https':
            self.handle = http.client.HTTPSConnection(self.url.hostname,
                                                  self.url.port)
        # direct, http
        else:
            self.handle = http.client.HTTPConnection(self.url.hostname,
                                                 self.url.port)
    ...
    def request(self, url, method="GET", body="", headers={}):
        ...
        if response.status == http.client.FORBIDDEN or \
